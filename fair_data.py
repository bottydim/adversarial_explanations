...
def read_german(filepath='../data/german.data-numeric',
    ...
    M = df.values
    # change from 9 to 8 due to 0 index
    z_idx = sensitive_property
    Z = M[:, z_idx]
    Z = Z.astype(float).reshape(-1, 1)
    ix = np.delete(np.arange(24), z_idx)
    X = M[:, ix]
    y = M[:, -1]
    ...
    tr_idx, te_idx = _get_train_test_split(n, train_frac, random_state,power_of_two=False)
